...
git** — since the system is distributed straight from the GitHub branch (via
the raw `system.json` manifest / branch-archive zip; no build step runs on the
end user's Foundry server), the repo must ship with already-compiled packs. (A
tagged-release workflow, `.github/workflows/release.yml`, also exists for
cutting versioned GitHub Releases later, but branch-tip is the active flow.)
